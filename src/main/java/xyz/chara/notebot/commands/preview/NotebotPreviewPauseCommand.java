...
public class NotebotPreviewPauseCommand {
                ...
                ClientCommandManager.literal("notebot")
                        .then(ClientCommandManager.literal("preview")
                                .then(ClientCommandManager.literal("pause")
                                        .executes(NotebotPreviewPauseCommand::run))));
    }

    private static int run(CommandContext<FabricClientCommandSource> context) {
        mc.player.sendMessage(
            Text.literal("§6Paused the preview of §a" + NotebotPreviewStartCommand.song.filename + "§6."));
        NotebotPreviewStartCommand.playing = false;
        return 1;
    }
}
